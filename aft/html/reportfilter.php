...
SELECT v.tid, v.verified, v.hks_request_date, v.verified_date, al.tid, aft.name, lb.name, lb.type, hk.hks_name, a.name, al.date_of_request, al.date_of_acceptance, al.date_of_lifting, al.quantity, al.seg_weight, al.non_seg_weight, al.invoice_id, al.invoice_date, al.amount, al.payment, al.date_of_payment, al.utr 
...
WHERE v.hks_request_date >= ? AND v.hks_request_date <= ? AND aft.id = ?";
...
$stmt->bind_param("sss", $from, $to, $aft);
...
$query = "SELECT v.tid, v.verified, v.hks_request_date, v.verified_date, al.tid, aft.name, lb.name, lb.type, hk.hks_name, a.name, al.date_of_request, al.date_of_acceptance, al.number_of_reminders 
...
$query = "SELECT v.tid, v.verified, v.hks_request_date, v.verified_date, aft.name, lb.name, lb.type, hk.hks_name, a.name, al.date_of_request, al.number_of_reminders, al.reminder_date 
...
$query = "SELECT v.tr_id, v.trackid, v.date, aft.name, lb.name, lb.type 
          ...
          WHERE v.date >= ? AND v.date <= ? AND v.trackid !='2' AND aft.id = ?";
...
while($rn=mysqli_fetch_array($result))
                         ...
                         {       
                           ...
                           echo"<tr><td>$rn[0]</td><td>$rn[2]</td><td>$rn[1]</td><td>$rn[3]</td><td>$rn[4]</td><td>$rn[5]</td></tr>";}}
