...
/*-1-*/$query = "SELECT al.tid,lb.name,lb.type,hk.hks_name,a.name,al.date_of_request,al.number_of_reminders,al.reminder_date FROM acceptance_pending al INNER JOIN hks hk on al.hksid=hk.id INNER JOIN mcf m ON hk.v1id=m.v1id INNER JOIN localbody lb ON m.lb_id=lb.id INNER JOIN aft ON lb.aft_id=aft.id INNER JOIN agency a ON al.agency_id=a.id WHERE lb.id=$lb";

               echo"<table>";

                     echo"<tr><td colspan='8'><h4>Lifting Request Acceptance Pending - Filtered On</h4></td></tr><td colspan='8'><h4>AFT: $aft_name | Agency:$agency_name | Local Body:$lb_name </h4></td></tr>";
